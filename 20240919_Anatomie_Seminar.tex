...
\usepackage[%
	backend=biber,
	style=ieee,% ieee is needed for hypercite at the bottomof the slides
	citetracker=true,
	isbn=false,
	url=false
	]{biblatex}
...
% Remove stuff from footcite/hypercite
...
\begin{frame}
	...
	\begin{columns}
		\begin{column}{0.6\textwidth}		
			Collaboration with \href{https://www.zmk.unibe.ch/}{zmk bern – Zahnmedizinische Kliniken}~\cite{Haberthuer2021,Wolf2021}\hypercite{Haberthuer2021},~\hypercite{Wolf2021}
			\begin{itemize}
 ...
 \only<2>{					  \node at (current page.center) {\includegraphics[width=\paperwidth]{./media/ZMK/tooth045/transparent-slices-rcs/image0000}};%
 }
 % Sticky note overlay from https://tex.stackexchange.com/a/159690/828
					  \node at (current page.center) [drop shadow={shadow xshift=2pt,shadow yshift=-4pt}, inner xsep=7pt, fill=yellow, xslant=-0.25, yslant=0.25, inner ysep=10pt] {\parbox[t]{3.5cm}{\emph{How to study the internals of +100 human teeth with X-ray microtomography}\\\\ November 12, 2020}};%
				%   \end{tikzpicture}%
			  %}
 	 \end{tikzpicture}%
 	 %}
	...
	\only<1>{%
		...
		\begin{tikzpicture}[remember picture,overlay]%
			\node at (current page.center) {%
				\animategraphics[autoplay,width=\paperwidth,every=\everyframe]{24}{./media/EAWAG/animation.head/104016.head.rec.animation.0}{000}{023}%
			};%
		\end{tikzpicture}%
	}%
		\only<2>{%
		...
		\begin{tikzpicture}[remember picture,overlay]%
			\node at (current page.center) {%
			\animategraphics[autoplay,width=\paperwidth,every=\everyframe]{24}{./media/EAWAG/animation.head/104016.head.rec.animation.0}{024}{073}%
			};%
		\end{tikzpicture}%			
		}%
		\only<3>{%
		...
		\begin{tikzpicture}[remember picture,overlay]%
			\node at (current page.center) {%
			\animategraphics[autoplay,width=\paperwidth,every=\everyframe]{24}{./media/EAWAG/animation.head/104016.head.rec.animation.0}{074}{099}%
			};%
		\end{tikzpicture}%
		}%
		\only<4>{%
		...
		\begin{tikzpicture}[remember picture,overlay]%
			\node at (current page.center) {%
			\animategraphics[autoplay,width=\paperwidth,every=\everyframe]{24}{./media/EAWAG/animation.head/104016.head.rec.animation.0}{100}{106}%
			};%
		\end{tikzpicture}%
		}%		
		\only<5>{%
		...
		\begin{tikzpicture}[remember picture,overlay]%
			\node at (current page.center) {%
			\animategraphics[autoplay,width=\paperwidth,every=\everyframe]{24}{./media/EAWAG/animation.head2pj/morphed0}{000}{099}%
			};%
		\end{tikzpicture}%
		}%
...
\begin{frame}
	...
	\begin{tikzpicture}[remember picture,overlay]%
			\node at (current page.center) {%
		\animategraphics[autoplay,width=\paperwidth,every=\everyframe]{24}{./media/EAWAG/animation.pj/104016.pj.animation.0}{000}{080}%
	};%
\end{tikzpicture}%	
...
\begin{frame}
	...
	\only<1>{%
		...
		\sourcelink{https://doi.org/10.1371/journal.pone.0112732}{DOI:gk6b4j}{Figure 1}%
		}%
	\only<2>{%
...
\begin{frame}
	...
	\begin{columns}
		\begin{column}{0.6\textwidth}		
			Collaboration with \emph{Developmental Biology and Regeneration} group of Nadia Mercader Huber~\cite{Garcia-Poyatos2024}\hypercite{Garcia-Poyatos2024}
			\begin{itemize}%
		...
		\begin{column}{0.4\textwidth}
			\only<1>{%
				\includegraphics[height=\imageheight]{./media/cox7a/Cell_Fig3}%
				}%
			\only<2>{%
